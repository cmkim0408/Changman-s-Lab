#!/usr/bin/env python3
"""Create the write-once R4.4 Stage-B lock before target reveal."""
...
def main() -> None:
    output = RECEIPTS / "R4_4_LOCAL_LOCK.json"
    if output.exists():
    ...
    if (
        ...
        or config.get("eligibility_failure_ids")
        != ELIGIBILITY_FAILURE_IDS
    ):
        raise RuntimeError("R4.4 config denominator/role mismatch")

    batch_path = RECEIPTS / "R4_4_PRE_REVEAL_BATCH.json"
    batch = read_json(batch_path)
    if (
        batch.get("status") != "R4_4_PRE_REVEAL_ROUTES_FROZEN"
        or batch.get("campaign_denominator") != CAMPAIGN_IDS
        or batch.get("eligibility_failure_denominator")
        != ELIGIBILITY_FAILURE_IDS
    ...
    if inferential_act_count < minimum_source_act:
        raise RuntimeError(
            "R4.4 pre-reveal viability failed: "
    ...
    if new_source_act_count < minimum_new_source_act:
        raise RuntimeError(
            "R4.4 pre-reveal new-data viability failed: "
    ...
    payload = {
        "schema": "cacl-oc-r4.4-local-lock-v1",
        "status": "R4_4_LOCKED_PENDING_EXTERNAL_TIMESTAMP",
