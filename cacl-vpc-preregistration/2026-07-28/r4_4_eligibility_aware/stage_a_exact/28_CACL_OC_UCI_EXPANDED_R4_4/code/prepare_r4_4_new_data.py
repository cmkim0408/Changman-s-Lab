#!/usr/bin/env python3
"""Prepare four pre-instance-data UCI tasks without target summaries."""
...
NEW_DATASETS = {
    367: {
        ...
        "raw_rows": 102944,
        "raw_features": 115,
        "primary_features": 115,
        "registered_row_cap": 100000,
        "raw_feature_name_sha256": (
            "be36b60a9a6dafa22e05e4dbb8267592e3c5fb8ae9f3a8935"
            "28a7e0718af0c21"
        ),
    ...
    891: {
        ...
        "registered_row_cap": 100000,
        "raw_feature_name_sha256": (
            "bae919eb203d535248c48300be955de0c13801e18ae89e5776a"
            "e9c1dbd7cadc3"
        ),
        "primary_feature_name_sha256": (
            "bae919eb203d535248c48300be955de0c13801e18ae89e5776a"
    ...
    942: {
        ...
        "registered_row_cap": 100000,
        "raw_feature_name_sha256": (
            "f10cfcc8643fb5a187aac5a16f64ba3f6f5b7b31f47722797e"
            "2e55dab2556704"
        ),
        "primary_feature_name_sha256": (
            "18b407e1a292f52d4f321593e238674819275cc14b2734b1ee"
            "2165118af792cf"
        ),
        "group_rule": "exact_primary_feature_signature",
    },
...
TRANSPORT = {
    367: {
        ...
        "metadata_snapshot_relative_path": (
            ...
            "metadata/uci_367.json"
        ),
        "metadata_snapshot_sha256": (
            "be6a782b8943d5276833158282bd3882633e4390e5431cbf4e8"
            "8db64de79ac49"
        ),
    ...
    891: {
        ...
        "metadata_snapshot_relative_path": (
            "22_CACL_VPC_UCI_BINARY_VERIFIED_BATCH/registry_snapshot/"
            "metadata/uci_891.json"
        ),
        "metadata_snapshot_sha256": (
            "68f14e4bdc9be4fa68eec2f064e5a8ebed033871e175f543c3"
            "e97add3e20bbdd"
        ),
        "redirect_policy": REDIRECT_POLICY,
    ...
    942: {
        ...
        "metadata_snapshot_relative_path": (
            "22_CACL_VPC_UCI_BINARY_VERIFIED_BATCH/registry_snapshot/"
            "metadata/uci_942.json"
        ),
        "metadata_snapshot_sha256": (
            "cac3def74da86e04053dc7fbaadcaa8cd658e1076079fc695a3"
            "1138e327e41a6"
        ),
        ...
        "taxonomy_registration_sha256": (
            "6d9aa1f859615356ec5c3f5402a2ed029c0f22a444bb6e0b5"
            "3f627e7fe5432b3"
        ),
        "redirect_policy": REDIRECT_POLICY,
        "maximum_csv_bytes": 512 * 1024 * 1024,
...
def verify_frozen_registration_config() -> None:
    ...
    if config.get("target_transforms") != {
        ...
        "942": {
            "estimand": (
                "official_exact_normal_labels_0_vs_official_exact_"
                "attack_labels_1"
            ),
            "normal_labels_after_outer_whitespace_trim": list(
                RTIOT_NORMAL_LABELS
            ),
            "attack_labels_after_outer_whitespace_trim": list(
                RTIOT_ATTACK_LABELS
            ),
        },
        "967": (
...
def require_stage_a_data_access_authorized() -> dict[str, Any]:
    ...
    verification = read_json(
        ROOT / "receipts" / "R4_4_STAGE_A_TIMESTAMP_VERIFICATION.json"
    )
    exact = (
...
def rtiot_binary_target(labels: pd.Series) -> tuple[np.ndarray, dict[str, int]]:
    """Apply the official, pre-instance exact class taxonomy."""

    taxonomy_path = PACKAGE / TRANSPORT[942][
        "taxonomy_registration_relative_path"
    ]
    if (
        not taxonomy_path.is_file()
        or sha256(taxonomy_path)
        != TRANSPORT[942]["taxonomy_registration_sha256"]
    ):
        raise RuntimeError("UCI 942: taxonomy registration changed")
    taxonomy = read_json(taxonomy_path)
    if (
        taxonomy.get("schema")
        != "cacl-oc-r4.4-uci942-official-taxonomy-v1"
        or taxonomy.get("uci_id") != 942
        or taxonomy.get("normal_labels_after_outer_whitespace_trim")
        != list(RTIOT_NORMAL_LABELS)
        or taxonomy.get("attack_labels_after_outer_whitespace_trim")
        != list(RTIOT_ATTACK_LABELS)
        or taxonomy.get("binary_encoding")
        != {"normal": 0, "attack": 1}
        or taxonomy.get(
            "instance_rows_accessed_to_create_this_registration"
        )
        is not False
    ):
        raise RuntimeError("UCI 942: taxonomy registration invalid")
    normal = set(RTIOT_NORMAL_LABELS)
    attack = set(RTIOT_ATTACK_LABELS)
    observed = labels.astype(str).str.strip()
    unknown = sorted(set(observed) - normal - attack)
    if unknown:
        raise RuntimeError(
            "UCI 942: target label absent from locked exact taxonomy"
        )
    values = [0 if label in normal else 1 for label in observed]
    result = np.asarray(values, dtype=int)
    if set(result.tolist()) != {0, 1}:
        raise RuntimeError("UCI 942: locked binary target lost one class")
    return result, {
...
def fetch_registered_dataset(dataset_id: int) -> tuple[
    ...
    request = Request(
        transport["url"],
        headers={"User-Agent": "CACL-OC-R4.4/1.0"},
    )
...
def build_prepared(dataset_id: int) -> dict[str, Any]:
    ...
    if dataset_id == 967:
        ...
        if not set(EXCLUDED_942).issubset(observed_feature_names):
            raise RuntimeError("UCI 942: locked feature exclusions changed")
        primary_names = [
            name for name in observed_feature_names
            if name not in EXCLUDED_942
        ]
    ...
    if dataset_id == 942:
        y, mapping = rtiot_binary_target(labels)
    else:
    ...
    receipt = {
        "schema": "cacl-oc-r4.4-new-data-preparation-v1",
        "status": "R4_4_NEW_DATASET_PREPARED",
        "uci_id": dataset_id,
        ...
        "target_transform_registration": (
            {
                "relative_path": TRANSPORT[942][
                    "taxonomy_registration_relative_path"
                ],
                "sha256": TRANSPORT[942][
                    "taxonomy_registration_sha256"
                ],
            }
            if dataset_id == 942
...
def main() -> None:
    ...
    output = ROOT / "receipts" / "R4_4_NEW_DATA_PREPARATION.json"
    if output.exists():
        raise FileExistsError(
    ...
    batch = {
        "schema": "cacl-oc-r4.4-new-data-preparation-batch-v1",
        "status": "R4_4_NEW_DATA_PREPARATION_COMPLETE",
