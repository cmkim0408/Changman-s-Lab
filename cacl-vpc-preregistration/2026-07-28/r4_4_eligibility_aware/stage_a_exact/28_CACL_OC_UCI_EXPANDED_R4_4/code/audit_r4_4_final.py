#!/usr/bin/env python3
"""Independent raw-data audit of the complete CACL-OC R4.4 campaign.
...
UCI942_EXCLUDED_FEATURES = ["proto", "service"]
UCI942_NORMAL_LABELS = [
    "MQTT",
    "Thing_speak",
    "Wipro_bulb_Dataset",
    "Amazon-Alexa",
]
UCI942_ATTACK_LABELS = [
    "DOS_SYN_Hping",
    "ARP_poisioning",
    "NMAP_UDP_SCAN",
    "NMAP_XMAS_TREE_SCAN",
...
def independent_new_target_transform(
    ...
    if dataset_id == 942:
        transport = config["transport"]["942"]
        taxonomy_path = (
            PACKAGE / transport["taxonomy_registration_relative_path"]
        )
        if (
            sha256(taxonomy_path)
            != transport["taxonomy_registration_sha256"]
        ):
            raise RuntimeError("independent taxonomy hash mismatch")
        taxonomy = read_json(taxonomy_path)
        if (
            taxonomy.get("normal_labels_after_outer_whitespace_trim")
            != UCI942_NORMAL_LABELS
            or taxonomy.get(
                "attack_labels_after_outer_whitespace_trim"
            )
            != UCI942_ATTACK_LABELS
            or taxonomy.get("binary_encoding")
            != {"normal": 0, "attack": 1}
        ):
            raise RuntimeError("independent taxonomy content mismatch")
        normal = set(UCI942_NORMAL_LABELS)
        attack = set(UCI942_ATTACK_LABELS)
        observed = labels.astype(str).str.strip()
        if set(observed) - normal - attack:
            raise RuntimeError(
                "independent target transform found an unknown class"
            )
        result = np.asarray(
            [0 if label in normal else 1 for label in observed],
            dtype=int,
        )
    else:
        numeric = pd.to_numeric(labels, errors="raise").to_numpy(
            dtype=float
        )
...
def independent_rebuild_new_preparation(
    ...
    if dataset_id == 967:
        ...
        primary_names = [
            ...
            if name not in UCI942_EXCLUDED_FEATURES
        ]
        raw_group_ids = np.asarray([], dtype=str)
    else:
        primary_names = feature_names
        raw_group_ids = np.asarray([], dtype=str)
    raw_x = (
    ...
    if dataset_id != 967:
        raw_group_ids = independent_feature_signature_groups(raw_x)
...
def audit_dataset(
    ...
    # is present.  Every new task except UCI 967 has the same registered
    # exact-signature rule and is likewise reconstructed from x.  UCI 967's
    # normalized-domain key is deliberately
    # excluded from x; for it, the locked NPZ identity is cross-checked across
    # source, feature, outcome, seal and preparation receipts below.
    ...
    if dataset_id in NEW_IDS and dataset_id != 967:
    ...
    if dataset_id in NEW_IDS:
        ...
        new_preparation_checks = {
            "schema_and_status": (
                ...
                == "cacl-oc-r4.4-new-data-preparation-v1"
                and preparation.get("status")
                == "R4_4_NEW_DATASET_PREPARED"
            ...
            "uci942_primary_hash_independently_recomputed": (
                dataset_id != 942
                or independent_name_list_sha256(
                    [
                        name
            ...
            "new_exact_feature_groups_recomputed": (
                dataset_id == 967
                or (
                    np.array_equal(
            ...
            "target_mapping_receipt_valid": (
                ...
                and (
                    mapping
                    == {
                        "rule": "pre_instance_official_exact_taxonomy",
                        "normal": 0,
                        "attack": 1,
                        "observed_target_vocabulary_disclosed": False,
                    }
                    if dataset_id == 942
                    else mapping
                    == {
                        "rule": (
                            "positive_numeric_value_to_1_"
                            "nonpositive_to_0"
                        ),
                        "mapping_fit_scope": (
                            "none_pre_instance_fixed_rule"
            ...
            "target_transform_receipt_exact": (
                ...
                == (
                    "official RT-IoT2022 exact normal-label set versus "
                    "official exact attack-label set"
                    if dataset_id == 942
                    else (
                ...
                and (
                    dataset_id != 942
                    or config.get("target_transforms", {}).get("942")
                    == {
                        "estimand": (
                            "official_exact_normal_labels_0_vs_"
                            "official_exact_attack_labels_1"
                        ),
                        "normal_labels_after_outer_whitespace_trim": (
                            UCI942_NORMAL_LABELS
                        ),
                        "attack_labels_after_outer_whitespace_trim": (
                            UCI942_ATTACK_LABELS
            ...
            "target_transform_registration_exact": (
                (
                    preparation.get("target_transform_registration")
                    is None
                )
                if dataset_id != 942
                else (
                    preparation.get("target_transform_registration")
                    == {
                        "relative_path": transport[
                            "taxonomy_registration_relative_path"
                        ],
                        "sha256": transport[
                            "taxonomy_registration_sha256"
                        ],
                    }
                    and sha256(
                        PACKAGE
    ...
    source_checks = {
        ...
        "source_schema": (
            ...
            == "cacl-oc-r4.4-source-freeze-receipt-v1"
            and frozen.get("schema")
            == "cacl-oc-r4.4-frozen-policy-v1"
        ),
        "dataset_ids_exact": (
    ...
    target_checks = {
        "verdict_schema": (
            verdict.get("schema")
            == "cacl-oc-r4.4-target-verdict-v1"
        ),
...
def main() -> None:
    output = RECEIPTS / "R4_4_FINAL_INTEGRITY_AUDIT.json"
    if output.exists():
    ...
    pre_reveal = read_json(RECEIPTS / "R4_4_PRE_REVEAL_BATCH.json")
    reveal_started_path = RECEIPTS / "R4_4_REVEAL_STARTED.json"
    reveal_completed_path = RECEIPTS / "R4_4_REVEAL_COMPLETED.json"
    final_path = RECEIPTS / "R4_4_FINAL_BATCH_VERDICT.json"
    ...
    if not reveal_start_authorized:
        raise RuntimeError(
            "R4.4 reveal-start receipt is invalid; refusing target decode"
        )
    ...
    batch_checks = {
        ...
        "config_denominators_exact": (
            ...
            and config.get("eligibility_failure_ids")
            == ELIGIBILITY_FAILURE_IDS
            ...
            and sorted(REGISTERED_IDS + ELIGIBILITY_FAILURE_IDS)
            == sorted(CAMPAIGN_IDS)
        ),
        ...
        "pre_reveal_schema_status": (
            ...
            == "cacl-oc-r4.4-pre-reveal-batch-v1"
            and pre_reveal.get("status")
            == "R4_4_PRE_REVEAL_ROUTES_FROZEN"
        ...
        "pre_reveal_denominators_exact": (
            ...
            and pre_reveal.get("eligibility_failure_denominator")
            == ELIGIBILITY_FAILURE_IDS
        ...
        "reveal_complete_write_once_receipt": (
            ...
            == "cacl-oc-r4.4-reveal-complete-v1"
            and reveal_completed.get("status")
            == "R4_4_ONE_SHOT_TARGET_REVEAL_COMPLETED"
            and reveal_completed.get("campaign_ids") == CAMPAIGN_IDS
            and reveal_completed.get("eligibility_failure_ids")
            == ELIGIBILITY_FAILURE_IDS
        ...
        "final_schema": (
            final.get("schema")
            == "cacl-oc-r4.4-final-batch-verdict-v1"
        ),
        "final_denominators_exact": (
            final.get("campaign_denominator") == CAMPAIGN_IDS
            and final.get("eligibility_failure_denominator")
            == ELIGIBILITY_FAILURE_IDS
    ...
    receipt = {
        "schema": "cacl-oc-r4.4-final-integrity-audit-v1",
        "status": (
            "R4_4_FINAL_INTEGRITY_AUDIT_PASS"
            if all_dataset_checks and all_batch_checks
            else "R4_4_FINAL_INTEGRITY_AUDIT_FAIL"
        ),
