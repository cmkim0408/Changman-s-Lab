#!/usr/bin/env python3
"""Fetch a public GitHub commit byte-for-byte before writing an R4.4 ACK."""
...
def fetch_bytes(url: str) -> bytes:
    request = Request(
        ...
        headers={
            ...
            "User-Agent": "CACL-R4.4-public-byte-verifier",
...
def mode_contract(mode: str) -> dict[str, Any]:
    if mode == "stage-a":
        return {
            "lock": RECEIPTS / "R4_4_STAGE_A_LOCAL_LOCK.json",
            "ack": RECEIPTS / "R4_4_STAGE_A_TIMESTAMP_ACK.json",
            "schema": "cacl-oc-r4.4-stage-a-timestamp-ack-v1",
            "unopened_field": "new_instance_data_unopened_at_timestamp",
        }
    return {
        "lock": RECEIPTS / "R4_4_LOCAL_LOCK.json",
        "ack": RECEIPTS / "R4_4_EXTERNAL_TIMESTAMP_ACK.json",
...
def verify_preconditions(mode: str) -> None:
    if mode == "stage-a":
        ...
        return
    forbidden = [
        RECEIPTS / "R4_4_REVEAL_STARTED.json",
        RECEIPTS / "R4_4_REVEAL_COMPLETED.json",
        RECEIPTS / "R4_4_FINAL_BATCH_VERDICT.json",
    ]
