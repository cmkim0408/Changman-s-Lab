#!/usr/bin/env python3
"""Shared write-once, snapshot and hash-chain checks for CACL-OC R4.3."""
...
def preparation_transport_matches(
    ...
    if dataset_id == 855:
        return (
            ...
            and acquisition.get("method") == registered.get("method")
            ...
            == registered.get("archive_member")
            and acquisition.get("target_column")
            == registered.get("target_column")
            and isinstance(acquisition.get("archive_bytes"), int)
            and 0
            < acquisition["archive_bytes"]
            <= int(registered["maximum_archive_bytes"])
            and isinstance(acquisition.get("csv_bytes"), int)
            and 0
            < acquisition["csv_bytes"]
            <= int(registered["maximum_csv_bytes"])
            and _sha256_text(acquisition.get("archive_sha256"))
...
def load_verified_target_snapshot(
    ...
    lock = read_json(RECEIPTS / "R4_3_LOCAL_LOCK.json")
...
def stage_a_required_paths() -> list[Path]:
    return [
        ...
        ROOT / "R4_3_STAGE_A_REGISTRATION.md",
        ROOT / "R4_3_PROTOCOL.md",
        ROOT / "requirements-lock.txt",
        ROOT / "config" / "r4_3_contract.json",
        ROOT / "code" / "integrity.py",
        ROOT / "code" / "prepare_r4_3_new_data.py",
        ROOT / "code" / "freeze_r4_3_stage_a_lock.py",
        ROOT / "code" / "verify_r4_3_stage_a_timestamp.py",
        ROOT / "code" / "create_r4_3_public_ack.py",
        ROOT / "code" / "cacl_oc_engine.py",
        ROOT / "code" / "freeze_r4_3_lock.py",
        ROOT / "code" / "verify_r4_3_timestamp.py",
        ROOT / "code" / "audit_r4_3_final.py",
    ]
...
def validate_r4_2_invalidation_provenance() -> dict[str, Any]:
    ...
    checks = {
        "r4_2_lock_schema_status": (
            ...
            and lock.get("new_registered_ids") == NEW_IDS
...
def validate_stage_a_ack_and_lock() -> dict[str, Any]:
    ...
    lock_path = RECEIPTS / "R4_3_STAGE_A_LOCAL_LOCK.json"
    ack_path = RECEIPTS / "R4_3_STAGE_A_TIMESTAMP_ACK.json"
    if not lock_path.is_file() or not ack_path.is_file():
        raise RuntimeError("R4.3 Stage-A lock or ACK is absent")
    lock = read_json(lock_path)
    ack = read_json(ack_path)
    expected_public_count = len(lock.get("public_text_paths", []))
    checks = _ack_checks(
    ...
    if not result["passes"]:
        failed = [name for name, value in checks.items() if not value]
        raise RuntimeError(
            "R4.3 Stage-A authorization failed: " + ", ".join(failed)
        )
    return result
...
def required_final_paths() -> list[Path]:
    paths = [
        ...
        ROOT / "code" / "verify_r4_3_timestamp.py",
        ROOT / "code" / "audit_r4_3_final.py",
        RECEIPTS / "R4_3_STAGE_A_LOCAL_LOCK.json",
        RECEIPTS / "R4_3_STAGE_A_TIMESTAMP_ACK.json",
        RECEIPTS / "R4_3_STAGE_A_TIMESTAMP_VERIFICATION.json",
        RECEIPTS / "R4_3_NEW_DATA_PREPARATION.json",
        RECEIPTS / "R4_3_PRE_REVEAL_BATCH.json",
...
def validate_ack_and_lock() -> dict[str, Any]:
    ...
    lock_path = RECEIPTS / "R4_3_LOCAL_LOCK.json"
    ack_path = RECEIPTS / "R4_3_EXTERNAL_TIMESTAMP_ACK.json"
    if not lock_path.is_file() or not ack_path.is_file():
        raise RuntimeError("R4.3 Stage-B lock or ACK is absent")
    lock = read_json(lock_path)
    ack = read_json(ack_path)
    config = read_json(ROOT / "config" / "r4_3_contract.json")
    batch = read_json(RECEIPTS / "R4_3_PRE_REVEAL_BATCH.json")
    expected_public_count = len(lock.get("public_text_paths", []))
    checks = _ack_checks(
        ack,
        lock_path,
        schema="cacl-oc-r4.3-timestamp-ack-v1",
        expected_public_count=expected_public_count,
        unopened_field=(
    ...
    checks.update(
        {
            ...
            "new_preparation_batch_exact": (
                ...
                == "cacl-oc-r4.3-new-data-preparation-batch-v1"
                and preparation_batch.get("status")
                == "R4_3_NEW_DATA_PREPARATION_COMPLETE"
            ...
            "lock_schema": (
                lock.get("schema") == "cacl-oc-r4.3-local-lock-v1"
            ),
            "lock_status": (
                lock.get("status")
                == "R4_3_LOCKED_PENDING_EXTERNAL_TIMESTAMP"
            ),
    ...
    if not result["passes"]:
        failed = [name for name, value in checks.items() if not value]
        raise RuntimeError(
            "R4.3 Stage-B authorization failed: " + ", ".join(failed)
        )
    return result
