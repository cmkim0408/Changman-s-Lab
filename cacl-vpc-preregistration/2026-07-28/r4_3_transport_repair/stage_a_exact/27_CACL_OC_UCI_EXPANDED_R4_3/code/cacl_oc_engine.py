#!/usr/bin/env python3
"""Frozen CACL-OC R4.3 engine with instrumented path access."""
...
CONFIG_PATH = ROOT / "config" / "r4_3_contract.json"
...
def freeze_one(dataset_id: int) -> dict[str, Any]:
    ...
    if set(np.unique(y)) != {0, 1}:
        raise RuntimeError("R4.3 requires a binary source task")
    ...
    frozen = {
        "schema": "cacl-oc-r4.3-frozen-policy-v1",
    ...
    receipt = {
        "schema": "cacl-oc-r4.3-source-freeze-receipt-v1",
...
def freeze_all() -> None:
    batch_path = ROOT / "receipts" / "R4_3_PRE_REVEAL_BATCH.json"
    ...
    batch = {
        "schema": "cacl-oc-r4.3-pre-reveal-batch-v1",
        "status": "R4_3_PRE_REVEAL_ROUTES_FROZEN",
...
def verify_timestamp_authorized() -> dict[str, Any]:
    ...
    verification_path = (
        ROOT / "receipts" / "R4_3_EXTERNAL_TIMESTAMP_VERIFICATION.json"
    )
    if not verification_path.exists():
        raise RuntimeError(
            "R4.3 external timestamp verification receipt is absent"
        )
...
def evaluate_one(dataset_id: int) -> dict[str, Any]:
    ...
    batch = read_json(
        ROOT / "receipts" / "R4_3_PRE_REVEAL_BATCH.json"
    )
    if (
    ...
    return {
        "schema": "cacl-oc-r4.3-target-verdict-v1",
        "dataset_id": int(dataset_id),
...
def evaluate_all() -> None:
    final_path = ROOT / "receipts" / "R4_3_FINAL_BATCH_VERDICT.json"
    reveal_started = ROOT / "receipts" / "R4_3_REVEAL_STARTED.json"
    reveal_completed = ROOT / "receipts" / "R4_3_REVEAL_COMPLETED.json"
    verdict_paths = [
        ROOT
        / "registered_census"
        / f"uci_{dataset_id}"
    ...
    write_json_once(
        reveal_started,
        {
            "schema": "cacl-oc-r4.3-reveal-start-v1",
            "status": "R4_3_ONE_SHOT_TARGET_REVEAL_STARTED",
    ...
    batch = {
        "schema": "cacl-oc-r4.3-final-batch-verdict-v1",
        "status": (
    ...
    write_json_once(
        reveal_completed,
        {
            "schema": "cacl-oc-r4.3-reveal-complete-v1",
            "status": "R4_3_ONE_SHOT_TARGET_REVEAL_COMPLETED",
