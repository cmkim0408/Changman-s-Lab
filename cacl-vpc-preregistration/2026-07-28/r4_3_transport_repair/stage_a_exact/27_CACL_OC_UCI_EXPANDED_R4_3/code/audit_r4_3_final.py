#!/usr/bin/env python3
"""Independent raw-data audit of the complete CACL-OC R4.3 campaign.
...
def audit_dataset(
    ...
    # is present.  UCI 855 has the same registered rule and is likewise
    # reconstructed from x.  UCI 967's normalized-domain key is deliberately
    ...
    if dataset_id in NEW_IDS:
        ...
        expected_excluded = (
            UCI967_EXCLUDED_FEATURES if dataset_id == 967 else []
        )
        ...
        new_preparation_checks = {
            "schema_and_status": (
                ...
                == "cacl-oc-r4.3-new-data-preparation-v1"
                and preparation.get("status")
                == "R4_3_NEW_DATASET_PREPARED"
            ...
            "transport_receipt_exact": (
                ...
                and (
                    (
                        ...
                        and acquisition.get("archive_member")
                        == transport["archive_member"]
                        and acquisition.get("target_column")
                        == transport["target_column"]
                        and isinstance(
                            acquisition.get("archive_bytes"), int
                        )
                        and 0
                        < acquisition["archive_bytes"]
                        <= transport["maximum_archive_bytes"]
                        and isinstance(acquisition.get("csv_bytes"), int)
                        and 0
                        < acquisition["csv_bytes"]
                        <= transport["maximum_csv_bytes"]
                        and independent_sha256_text(
                            acquisition.get("archive_sha256")
                        )
                        and independent_sha256_text(
            ...
            "uci855_exact_feature_groups_recomputed": (
                dataset_id != 855
                or (
                    np.array_equal(
    ...
    source_checks = {
        ...
        "source_schema": (
            ...
            == "cacl-oc-r4.3-source-freeze-receipt-v1"
            and frozen.get("schema")
            == "cacl-oc-r4.3-frozen-policy-v1"
        ),
        "dataset_ids_exact": (
    ...
    target_checks = {
        "verdict_schema": (
            verdict.get("schema")
            == "cacl-oc-r4.3-target-verdict-v1"
        ),
...
def main() -> None:
    output = RECEIPTS / "R4_3_FINAL_INTEGRITY_AUDIT.json"
    if output.exists():
    ...
    verification = read_json(
        RECEIPTS / "R4_3_EXTERNAL_TIMESTAMP_VERIFICATION.json"
    )
    pre_reveal = read_json(RECEIPTS / "R4_3_PRE_REVEAL_BATCH.json")
    reveal_started_path = RECEIPTS / "R4_3_REVEAL_STARTED.json"
    reveal_completed_path = RECEIPTS / "R4_3_REVEAL_COMPLETED.json"
    final_path = RECEIPTS / "R4_3_FINAL_BATCH_VERDICT.json"
    ...
    reveal_start_authorized = (
        ...
        == "cacl-oc-r4.3-reveal-start-v1"
        and reveal_started.get("status")
        == "R4_3_ONE_SHOT_TARGET_REVEAL_STARTED"
    ...
    if not reveal_start_authorized:
        raise RuntimeError(
            "R4.3 reveal-start receipt is invalid; refusing target decode"
        )
    ...
    expected_status = (
        "R4_3_CONFIRMATORY_PASS"
        if all(expected_gates.values())
        else "R4_3_CONFIRMATORY_FAIL"
    )
    ...
    batch_checks = {
        ...
        "timestamp_verification_exact": (
            ...
            == "cacl-oc-r4.3-timestamp-verification-v1"
            and verification.get("status")
            == "R4_3_TARGET_REVEAL_AUTHORIZED"
        ...
        "reveal_complete_write_once_receipt": (
            ...
            == "cacl-oc-r4.3-reveal-complete-v1"
            and reveal_completed.get("status")
            == "R4_3_ONE_SHOT_TARGET_REVEAL_COMPLETED"
        ...
        "final_schema": (
            final.get("schema")
            == "cacl-oc-r4.3-final-batch-verdict-v1"
        ),
        "final_denominators_exact": (
            final.get("registered_denominator") == REGISTERED_IDS
    ...
    receipt = {
        "schema": "cacl-oc-r4.3-final-integrity-audit-v1",
        "status": (
            "R4_3_FINAL_INTEGRITY_AUDIT_PASS"
            if all_dataset_checks and all_batch_checks
            else "R4_3_FINAL_INTEGRITY_AUDIT_FAIL"
        ),
