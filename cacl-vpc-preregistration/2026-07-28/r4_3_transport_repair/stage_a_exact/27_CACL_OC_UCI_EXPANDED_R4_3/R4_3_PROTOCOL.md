# CACL-OC R4.3: pre-data-registered, integrity-hardened UCI campaign
...
R4.2's Stage-A code was publicly timestamped and fetched back correctly, but
UCI 855 then stopped before instance-data access because the UCI metadata API
exposed no `data_url`. R4.2 was locally invalidated before instance access; it
produced no prepared dataset and no scientific result. R4.3 is a
transport-only repair whose Stage-A lock binds and publicly timestamps that
invalidation record. It fixes the exact official UCI 855 ZIP
URL, member name, target column and byte caps before access. All scientific
choices remain unchanged.
Any HTTP redirect must preserve the HTTPS origin and decoded archive path and
must add neither query nor fragment; otherwise preparation fails closed.
In particular, every scientific selection/group/split hash uses its exact
R4.2 namespace; R4.3 versioning applies only to the repaired transport and
new integrity receipts.
...
4. preserves the requirement for at least two inferential source ACT routes
   and at least two target ACT passes from policies and actions frozen without
   target outcomes; and
...
241-bit feature signature. UCI 855 is read from the locked official static ZIP
member `TUANDROMD.csv`; UCI 967 uses the locked UCI API client. A transport,
archive-member or schema mismatch stops the campaign rather than choosing a
replacement source. Whole groups, not rows, are assigned to one side.
...
lock is published, fetched back from GitHub and byte-compared by the verifier.
Only then may
...
`P(O=1) >= (1-eta)kappa`. R4.3 retains the harm ceiling and certifies useful
...
`R4_3_CONFIRMATORY_PASS` requires the complete five-dataset denominator,
...
- `create_r4_3_public_ack.py` fetches the named GitHub commit and every
...
computational confirmation on at least two registered inferential UCI
group-representative tasks whose policy fitting, routing and target actions
were target-outcome-uninformed. The preparation layer decoded labels in the
same local environment to create separated outcome archives, but the
scientific engine did not semantically open those archives until the locked
one-shot reveal. The evidence also includes instrumented tree-path access
replay. The newly added UCI 855 and 967 are registered before instance-level
...
The UCI 855 archive and CSV byte hashes and sizes are recorded and checked
against the locked transport receipt, but the raw ZIP is not retained, so
those two transport hashes cannot later be recomputed from this package.
