#!/usr/bin/env python3
"""Create the write-once R4.2 Stage-B lock before target reveal."""
...
def main() -> None:
    output = RECEIPTS / "R4_2_LOCAL_LOCK.json"
    if output.exists():
    ...
    if (
        ...
        raise RuntimeError("R4.2 config denominator/role mismatch")

    batch_path = RECEIPTS / "R4_2_PRE_REVEAL_BATCH.json"
    batch = read_json(batch_path)
    if (
        batch.get("status") != "R4_2_PRE_REVEAL_ROUTES_FROZEN"
    ...
    if inferential_act_count < minimum_source_act:
        raise RuntimeError(
            "R4.2 pre-reveal viability failed: "
    ...
    if new_source_act_count < minimum_new_source_act:
        raise RuntimeError(
            "R4.2 pre-reveal new-data viability failed: "
    ...
    payload = {
        "schema": "cacl-oc-r4.2-local-lock-v1",
        "status": "R4_2_LOCKED_PENDING_EXTERNAL_TIMESTAMP",
