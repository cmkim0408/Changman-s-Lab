...
def build_prepared(dataset_id: int) -> dict[str, Any]:
    ...
    receipt = {
        "schema": "cacl-oc-r4.2-new-data-preparation-v1",
        "status": "R4_2_NEW_DATASET_PREPARED",
        "uci_id": dataset_id,
        "registered_name": expected["name"],
        "observed_name": str(
            getattr(dataset.metadata, "name", dataset_id)
        ),
        "raw_rows": len(x),
...
def main() -> None:
    ...
    verification = read_json(
        ROOT / "receipts" / "R4_2_STAGE_A_TIMESTAMP_VERIFICATION.json"
    )
    if (
        ...
        raise RuntimeError("Stage-A verification is absent or inconsistent")
    output = ROOT / "receipts" / "R4_2_NEW_DATA_PREPARATION.json"
    if output.exists():
        raise FileExistsError(
    ...
    batch = {
        "schema": "cacl-oc-r4.2-new-data-preparation-batch-v1",
        "status": "R4_2_NEW_DATA_PREPARATION_COMPLETE",
