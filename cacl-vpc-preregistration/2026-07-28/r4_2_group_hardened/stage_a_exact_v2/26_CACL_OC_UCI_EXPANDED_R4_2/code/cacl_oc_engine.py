#!/usr/bin/env python3
"""Frozen CACL-OC R4.2 engine with instrumented path access."""
...
CONFIG_PATH = ROOT / "config" / "r4_2_contract.json"
...
def freeze_one(dataset_id: int) -> dict[str, Any]:
    ...
    if set(np.unique(y)) != {0, 1}:
        raise RuntimeError("R4.2 requires a binary source task")
    ...
    frozen = {
        "schema": "cacl-oc-r4.2-frozen-policy-v1",
    ...
    receipt = {
        "schema": "cacl-oc-r4.2-source-freeze-receipt-v1",
...
def freeze_all() -> None:
    batch_path = ROOT / "receipts" / "R4_2_PRE_REVEAL_BATCH.json"
    ...
    batch = {
        "schema": "cacl-oc-r4.2-pre-reveal-batch-v1",
        "status": "R4_2_PRE_REVEAL_ROUTES_FROZEN",
...
def verify_timestamp_authorized() -> dict[str, Any]:
    ...
    verification_path = (
        ROOT / "receipts" / "R4_2_EXTERNAL_TIMESTAMP_VERIFICATION.json"
    )
    if not verification_path.exists():
        raise RuntimeError(
            "R4.2 external timestamp verification receipt is absent"
        )
    verification = read_json(verification_path)
    if (
        verification.get("status")
        != "R4_2_TARGET_REVEAL_AUTHORIZED"
        or verification.get("lock_sha256") != chain["lock_sha256"]
        or verification.get("ack_sha256") != chain["ack_sha256"]
        or verification.get("public_commit_sha")
        != chain["public_commit_sha"]
    ):
        raise RuntimeError(
            "verification receipt disagrees with recomputed hash chain"
        )
    return chain
...
def evaluate_one(dataset_id: int) -> dict[str, Any]:
    ...
    batch = read_json(
        ROOT / "receipts" / "R4_2_PRE_REVEAL_BATCH.json"
    )
    if (
    ...
    return {
        "schema": "cacl-oc-r4.2-target-verdict-v1",
        "dataset_id": int(dataset_id),
...
def evaluate_all() -> None:
    final_path = ROOT / "receipts" / "R4_2_FINAL_BATCH_VERDICT.json"
    reveal_started = ROOT / "receipts" / "R4_2_REVEAL_STARTED.json"
    reveal_completed = ROOT / "receipts" / "R4_2_REVEAL_COMPLETED.json"
    verdict_paths = [
        ROOT
        / "registered_census"
        / f"uci_{dataset_id}"
    ...
    write_json_once(
        reveal_started,
        {
            "schema": "cacl-oc-r4.2-reveal-start-v1",
            "status": "R4_2_ONE_SHOT_TARGET_REVEAL_STARTED",
    ...
    batch = {
        "schema": "cacl-oc-r4.2-final-batch-verdict-v1",
        "status": (
    ...
    write_json_once(
        reveal_completed,
        {
            "schema": "cacl-oc-r4.2-reveal-complete-v1",
            "status": "R4_2_ONE_SHOT_TARGET_REVEAL_COMPLETED",
