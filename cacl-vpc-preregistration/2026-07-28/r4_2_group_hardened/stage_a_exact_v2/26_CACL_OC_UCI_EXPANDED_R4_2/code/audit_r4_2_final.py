#!/usr/bin/env python3
"""Independent raw-data audit of the complete CACL-OC R4.2 campaign.
...
def audit_dataset(
    ...
    if dataset_id in NEW_IDS:
        ...
        new_preparation_checks = {
            "schema_and_status": (
                ...
                == "cacl-oc-r4.2-new-data-preparation-v1"
                and preparation.get("status")
                == "R4_2_NEW_DATASET_PREPARED"
    ...
    source_checks = {
        ...
        "source_schema": (
            ...
            == "cacl-oc-r4.2-source-freeze-receipt-v1"
            and frozen.get("schema")
            == "cacl-oc-r4.2-frozen-policy-v1"
        ),
        "dataset_ids_exact": (
    ...
    target_checks = {
        "verdict_schema": (
            verdict.get("schema")
            == "cacl-oc-r4.2-target-verdict-v1"
        ),
...
def main() -> None:
    output = RECEIPTS / "R4_2_FINAL_INTEGRITY_AUDIT.json"
    if output.exists():
    ...
    verification = read_json(
        RECEIPTS / "R4_2_EXTERNAL_TIMESTAMP_VERIFICATION.json"
    )
    pre_reveal = read_json(RECEIPTS / "R4_2_PRE_REVEAL_BATCH.json")
    reveal_started_path = RECEIPTS / "R4_2_REVEAL_STARTED.json"
    reveal_completed_path = RECEIPTS / "R4_2_REVEAL_COMPLETED.json"
    final_path = RECEIPTS / "R4_2_FINAL_BATCH_VERDICT.json"
    ...
    reveal_start_authorized = (
        ...
        == "cacl-oc-r4.2-reveal-start-v1"
        and reveal_started.get("status")
        == "R4_2_ONE_SHOT_TARGET_REVEAL_STARTED"
    ...
    if not reveal_start_authorized:
        raise RuntimeError(
            "R4.2 reveal-start receipt is invalid; refusing target decode"
        )
    ...
    expected_status = (
        "R4_2_CONFIRMATORY_PASS"
        if all(expected_gates.values())
        else "R4_2_CONFIRMATORY_FAIL"
    )
    ...
    stale_r4_1_outputs = sorted(
        path.name for path in RECEIPTS.glob("R4_1_*")
    )
    ...
    batch_checks = {
        ...
        "pre_reveal_schema_status": (
            ...
            == "cacl-oc-r4.2-pre-reveal-batch-v1"
            and pre_reveal.get("status")
            == "R4_2_PRE_REVEAL_ROUTES_FROZEN"
        ...
        "timestamp_verification_exact": (
            ...
            == "cacl-oc-r4.2-timestamp-verification-v1"
            and verification.get("status")
            == "R4_2_TARGET_REVEAL_AUTHORIZED"
        ...
        "reveal_complete_write_once_receipt": (
            ...
            == "cacl-oc-r4.2-reveal-complete-v1"
            and reveal_completed.get("status")
            == "R4_2_ONE_SHOT_TARGET_REVEAL_COMPLETED"
        ...
        "final_schema": (
            final.get("schema")
            == "cacl-oc-r4.2-final-batch-verdict-v1"
        ),
        ...
        == expected_status,
        "no_stale_r4_1_outputs": not stale_r4_1_outputs,
    }
    all_dataset_checks = all(row["passes"] for row in rows)
    all_batch_checks = all(batch_checks.values())
    receipt = {
        "schema": "cacl-oc-r4.2-final-integrity-audit-v1",
        "status": (
            "R4_2_FINAL_INTEGRITY_AUDIT_PASS"
            if all_dataset_checks and all_batch_checks
            else "R4_2_FINAL_INTEGRITY_AUDIT_FAIL"
        ),
        ...
        "stale_r4_1_outputs": stale_r4_1_outputs,
