#!/usr/bin/env python3
"""Frozen CACL-OC R4.5 engine with instrumented path access."""
...
CONFIG_PATH = ROOT / "config" / "r4_5_contract.json"
...
def freeze_one(dataset_id: int) -> dict[str, Any]:
    ...
    if set(np.unique(y)) != {0, 1}:
        raise RuntimeError("R4.5 requires a binary source task")
    ...
    receipt = {
        "schema": "cacl-oc-r4.5-source-freeze-receipt-v1",
...
def freeze_all() -> None:
    batch_path = ROOT / "receipts" / "R4_5_PRE_REVEAL_BATCH.json"
    ...
    batch = {
        "schema": "cacl-oc-r4.5-pre-reveal-batch-v1",
        "status": "R4_5_PRE_REVEAL_ROUTES_FROZEN",
...
def evaluate_one(dataset_id: int) -> dict[str, Any]:
    ...
    batch = read_json(
        ROOT / "receipts" / "R4_5_PRE_REVEAL_BATCH.json"
    )
    if (
    ...
    return {
        "schema": "cacl-oc-r4.5-target-verdict-v1",
        "dataset_id": int(dataset_id),
...
def evaluate_all() -> None:
    final_path = ROOT / "receipts" / "R4_5_FINAL_BATCH_VERDICT.json"
    reveal_started = ROOT / "receipts" / "R4_5_REVEAL_STARTED.json"
    reveal_completed = ROOT / "receipts" / "R4_5_REVEAL_COMPLETED.json"
    verdict_paths = [
        ROOT
        / "registered_census"
        / f"uci_{dataset_id}"
    ...
    write_json_once(
        reveal_started,
        {
            "schema": "cacl-oc-r4.5-reveal-start-v1",
            "status": "R4_5_ONE_SHOT_TARGET_REVEAL_STARTED",
    ...
    batch = {
        "schema": "cacl-oc-r4.5-final-batch-verdict-v1",
        "status": (
    ...
    write_json_once(
        reveal_completed,
        {
            "schema": "cacl-oc-r4.5-reveal-complete-v1",
            "status": "R4_5_ONE_SHOT_TARGET_REVEAL_COMPLETED",
