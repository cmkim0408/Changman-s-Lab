#!/usr/bin/env python3
"""Hash-lock R4.5 dataset registration before new-data access."""
...
def main() -> None:
    output = RECEIPTS / "R4_5_STAGE_A_LOCAL_LOCK.json"
    if output.exists():
    ...
    payload = {
        "schema": "cacl-oc-r4.5-stage-a-local-lock-v1",
        "status": "R4_5_STAGE_A_LOCKED_BEFORE_NEW_DATA_ACCESS",
        "new_registered_ids": NEW_IDS,
        "new_data_downloaded": False,
        "prior_access_audit_sha256": prior_access["sha256"],
        "prelock_static_audit_v2_sha256": prelock["sha256"],
        ...
        "public_text_paths": [
            *[
                ...
                for relative, path in zip(
                    relative_files, files, strict=True
                )
